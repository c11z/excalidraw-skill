...
def render(
    ...
    # Lint before rendering: structural ERRORs block, WARNs are informational
    try:
        from lint_excalidraw import lint_data, load_palette, report
    except ImportError:
        pass
    else:
        findings = lint_data(data, load_palette())
        if findings:
            report(excalidraw_path, findings, sys.stderr)
        if any(f.severity == "ERROR" for f in findings):
            print("ERROR: Lint found structural errors — fix them before rendering.", file=sys.stderr)
            sys.exit(1)
    ...
    with sync_playwright() as p:
        ...
        # esm.sh serves the bundle as many submodule imports; cold loads can be slow
